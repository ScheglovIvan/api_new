...
var Layout = (function() {
    ...
    function copyClickBoard() {
            ...
            copyLink.addEventListener('click', function (e) {
                e.stopPropagation();
                e.preventDefault();
                const copyText = document.querySelector("input[name='special_link']");
    ...
    $('.side-custom-toggle').on('click', function (e) {
        $('.custom-nav-application').toggleClass('mobile-hide');
        if($(window).width() < 576) {
            $('.content-mobile').toggleClass('content-mobile-show');
        }
...
var EngagementChart = (function() {
	...
	function init($this) {
		...
		var options = {
			...
			series: [{
				name: 'Подписок',
				data: $($chart).attr('data-value').split(',')
			}],
			xaxis: {
				labels: {
                    format: 'MMM',
